...
/**
 * A MessageRetriever which retrieves information from the Crews Service within the Proxy Applications
 */
@Entity
public class CrewsRetriever extends MessageRetriever{
    /**
     ...
     *
     */
    public CrewsRetriever(String source, String username, String password, RetrieverOwner owner) {
        super(source, owner);
    ...
    public Message[] getAll() {
        ...
        HttpRequest request = HttpRequest.newBuilder().GET().uri(URI.create(source+"?sortBy=date")).setHeader("X-Crews-API",authString).build();
